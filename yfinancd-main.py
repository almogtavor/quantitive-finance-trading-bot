...
def should_sell(five_minute_ma, stop_loss, current_price, rsi, last_rsi_values, threshold=0.005):
    ...
    # Print the reason for selling
    if current_price <= stop_loss:
        print(f"SELL reason: Price below stop loss.")
    elif five_minute_ma[-1] < five_minute_ma[-3] * (1 - threshold):
        print(f"SELL reason: 5-minute MA trending down significantly.")
    elif rsi[-1] > dynamic_rsi_threshold:
        print(f"SELL reason: RSI overbought. RSI: {rsi[-1]}, Dynamic threshold: {dynamic_rsi_threshold}")
    # Sell if the current price is less than the stop loss,
    # or if the 5-minute MA is trending down significantly,
    # or if the RSI indicates the stock is overbought.
    # Adjust the threshold for percentage_change as needed.
    return (current_price <= stop_loss or
            five_minute_ma[-1] < five_minute_ma[-3] * (1 - threshold) or
            rsi[-1] > dynamic_rsi_threshold)
...
if __name__ == "__main__":
    ...
    for i in range(60, len(prices)):
        ...
        if not in_position:
            ...
            if should_sell(five_minute_ma, stop_loss, current_price, rsi, last_rsi_values, SELL_THRESHOLD):
